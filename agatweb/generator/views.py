...
def TasksView(request, task_id):
    ...
    tasks_list = Task.objects.all()
    gen_list = GenTask.objects.filter(user=request.user, taskId=task)
    return render(request, 'generator/index.html', {'tasks_list': tasks_list, 'task': curtask, 'gen_list': gen_list})
...
def DownloadTask(request, task_id):
    if not request.user.is_authenticated:
        return redirect('/')
    task = get_object_or_404(GenTask, user=request.user, instance=int(request.GET['id']))
    path = os.path.join(os.path.abspath(os.getcwd()), 'agatweb',  'static', 'Stud',
                        ...
                        '{}{}{}{}'.format(request.GET['id'], request.user.last_name, request.user.first_name, '.pdf'))
